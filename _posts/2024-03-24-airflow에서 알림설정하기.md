...
title: 'Airflow 알림 설정하기'
excerpt: ' Airflow에서 Slack, Email, Webhook 알림 설정하기 😎'
tags: [Airflow]
---
...
* Airflow는 Task 실패, 성공, 재시도 등의 이벤트 발생 시 다양한 채널로 알림을 보낼 수 있음.
* 콜백 함수를 사용하여 Slack, Email, Webhook, PagerDuty 등으로 알림을 통합할 수 있음.
...
1. **Callback 함수**는 Task 실행 후 자동으로 호출
*  `on_failure_callback`: Task 실패 시 호출
*  `on_success_callback`: Task 성공 시 호출
*  `on_retry_callback`: Task 재시도 시 호출
*  `sla_miss_callback`: SLA 미충족 시 호출
2. DAG 레벨 또는 Task 레벨에서 설정 가능함.
...
critical_task = PythonOperator(
    ...
    on_success_callback=slack_success_alert,  # Task 레벨 콜백
    dag=dag,
)
```
...
send_report = EmailOperator(
    ...
    <p>Status: Success</p>
    """,
    files=['/tmp/daily_report_{{ ds }}.pdf'],  # 첨부 파일
    dag=dag,
)
```

### 3. Webhook 알림 (통합 알림 시스템)
...
def webhook_alert(context, status='failure'):
    """Webhook으로 알림 전송 (PagerDuty, OpsGenie 등)"""
    task_instance = context.get('task_instance')
    exception = context.get('exception')
...
1. **알림 우선순위**: 모든 실패에 알림을 보내면 매우 피곤함. 중요한 DAG/Task만 알림하거나, 최종 실패 시에만 알림하도록 함.
2. **컨텍스트 정보**: 로그 URL, 실행 시간, 에러 메시지 등 디버깅에 필요한 정보를 포함함.
3. **알림 채널 분리**: 일반 알림은 Slack, 긴급 알림은 PagerDuty로 분리하여 관리함.
4. **Fallback 메커니즘**: 한 채널이 실패해도 다른 채널로 알림이 가도록 예외 처리함.
5. **테스트**: 알림이 제대로 작동하는지 주기적으로 테스트
