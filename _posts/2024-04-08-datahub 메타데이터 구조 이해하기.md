...
title: 'DataHub의 메타데이터 모델 구조 이해하기'
excerpt: ' Datahub 시작, 적용해보기 😎'
tags: [Datahub]
---


# 4. DataHub의 메타데이터 모델 구조 이해하기

## 개념

* DataHub는 Entity-Aspect 모델을 사용하여 메타데이터를 구조화 함.
* 이 모델은 유연하고 확장 가능하며, 다양한 타입의 메타데이터를 일관된 방식으로 저장하고 조회할 수 있도록 해줌.

## 핵심 개념 정리

**1. Entity (엔티티)** 메타데이터의 주요 객체를 나타냄.

```
주요 Entity 타입:
- Dataset: 테이블, 뷰, 파일 등의 데이터셋
- DataJob: Airflow Task, Spark Job 등의 작업
- DataFlow: Airflow DAG, Pipeline 등의 워크플로우
- Dashboard: BI 대시보드
- Chart: 대시보드 내 차트
- MLModel: 머신러닝 모델
- MLFeature: 피처
- Container: 데이터베이스, 스키마 등의 컨테이너
- Domain: 비즈니스 도메인
- Tag: 태그
- GlossaryTerm: 용어 사전
- CorpUser: 사용자
...
**2. Aspect (애스펙트)** Entity의 특정 속성이나 메타데이터를 나타냄.

```
주요 Aspect 타입:
- datasetProperties: 데이터셋 기본 정보 (설명, 생성일 등)
- schemaMetadata: 스키마 정보 (컬럼, 타입 등)
- ownership: 소유자 정보
- globalTags: 태그
- glossaryTerms: 용어 사전 연결
- upstreamLineage: 업스트림 계보
- institutionalMemory: 문서, 링크
- status: 삭제 여부
- datasetProfile: 통계 정보
```

**3. URN (Uniform Resource Name)** Entity를 고유하게 식별하는 ID
...
└─────────────────────────────────────────────────────┘
  ...
  │Properties│  │ Schema   │  │Ownership │  │   ...    │
  └──────────┘  └──────────┘  └──────────┘  └──────────┘
```

## Dataset Entity 상세 구조
...
lineage = graph.get_lineage(
    entity_urn=dataset_urn,
    direction='UPSTREAM',  # 또는 'DOWNSTREAM'
    max_hops=3,
)
...
update_description(dataset_urn, '고객 마스터 데이터')
...
def hard_delete_dataset(dataset_urn):
    ...
    # Hard delete는 REST API 직접 호출 필요
    import requests
...
def add_data_quality_metrics(dataset_urn, quality_data):
    ...
    # 커스텀 Aspect를 customProperties로 저장
    properties = graph.get_aspect(
        entity_urn=dataset_urn,
        aspect_type=DatasetPropertiesClass,
    )
...
```python
# Aspect에는 version이 있어 변경 이력 추적 가능
...
1. **URN 표준화**: 조직 내 URN 명명 규칙을 정하고 일관되게 사용
2. **Aspect 최소화**: 필요한 Aspect만 사용하여 복잡도 감소
3. **배치 처리**: 여러 Entity 업데이트 시 배치로 처리하여 성능 향상
4. **Soft Delete 우선**: Hard Delete는 최후의 수단으로 사용
5. **버전 관리**: 중요한 메타데이터는 변경 이력을 주기적으로 확인
6. **커스텀 확장**: 표준 Aspect로 부족하면 customProperties 활용
