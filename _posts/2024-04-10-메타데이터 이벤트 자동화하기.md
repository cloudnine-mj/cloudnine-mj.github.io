...
title: 'DataHub + Actions로 메타데이터 이벤트 자동화'
...
# DataHub + Actions로 메타데이터 이벤트 자동화

## 개념

* DataHub Actions는 메타데이터 변경 이벤트를 감지하고 자동으로 특정 작업을 수행하는 프레임워크
* Kafka를 통해 전송되는 메타데이터 이벤트를 구독하여 Slack 알림, Jira 이슈 생성, 데이터 품질 체크 등을 자동화할 수 있음.
...
## Actions 설정에 커스텀 Action 추가


```yaml
# actions_config.yml (계속)
actions:
...
class AutoQualityCheckAction(Action):
    ...
    def _run_quality_checks(self, dataset_urn: str) -> dict:
        ...
        # 예제: 실제로는 Great Expectations, Deequ 등 사용
...
1. **필터링**: 모든 이벤트에 반응하지 말고 필요한 이벤트만 필터링
2. **멱등성**: Action이 여러 번 실행되어도 문제없도록 설계
3. **에러 처리**: Action 실패 시 재시도 및 로깅
4. **성능**: 무거운 작업은 비동기로 처리
5. **테스트**: 프로덕션 적용 전 충분한 테스트
6. **모니터링**: Action 실행 로그 및 성공/실패율 모니터링