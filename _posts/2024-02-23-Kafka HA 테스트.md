...
$ bin/kafka-run-class.sh kafka.tools.GetOffsetShell --broker-list localhost:9092 --topic test-topic --time -1
Error: partition 2 does not have a leader. Skip getting offsets
[2024-02-21 01:00:45,181] WARN [Consumer clientId=GetOffsetShell, groupId=null] 1 partitions have leader brokers without a matching listener, including [test-topic-2] (org.apache.kafka.clients.NetworkClient)
test-topic:0:16
test-topic:1:24 
```
...
- node 1+2 shutdown 
	* kafka는 살아있으므로 문제는 없음, 단 남은 하나의 주키퍼에서 에러가 뜸
...
```
[2024-02-21 08:27:28,483] WARN Cannot open channel to 1 at election address /192.168.0.155:3888 (org.apache.zookeeper.server.quorum.QuorumCnxManager)
```
...
- node의 interface를 그냥 꺼버리는 형식으로 테스트해봄.
