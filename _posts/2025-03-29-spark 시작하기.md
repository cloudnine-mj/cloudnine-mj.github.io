---
key: jekyll-text-theme
title: 'Apache Spark 시작하기'
excerpt: ' Spark study 😎'
tags: [Spark]
---

# Apache Spark 설치하기

## Spark란?

Apache Spark는 대용량 데이터 처리를 위한 빠르고 범용적인 분산 처리 엔진임. Hadoop MapReduce보다 메모리 기반 처리로 최대 100배 빠른 성능을 제공하며, Java, Scala, Python, R 등 다양한 언어를 지원함.

**Spark vs Hadoop MapReduce**

| 특성        | Spark                     | Hadoop MapReduce        |
|-------------|---------------------------|--------------------------|
| 처리 방식   | 메모리 기반               | 디스크 기반             |
| 처리 속도   | 매우 빠름 (10~100배)      | 느림                     |
| 학습 곡선   | 쉬움                      | 어려움                   |
| 실시간 처리 | 지원 (Streaming)          | 지원 안 함               |
| 반복 연산   | 최적화됨 (ML)             | 비효율적                 |


## 설치 방법 1: Standalone 모드 (단일 서버)

**사전 요구사항**

- Java 8 또는 11
- Scala 2.12 (선택사항)
- Python 3.7 이상 (PySpark 사용 시)

**1단계: Java 설치 확인**

```bash
# Java 버전 확인
java -version

# Java가 없다면 설치
sudo apt update
sudo apt install openjdk-11-jdk -y

# JAVA_HOME 설정
echo 'export JAVA_HOME=/usr/lib/jvm/java-11-openjdk-amd64' >> ~/.bashrc
source ~/.bashrc
```

**2단계: Spark 다운로드 및 설치**

```bash
# Spark 다운로드 (3.5.0 버전 기준)
wget https://dlcdn.apache.org/spark/spark-3.5.0/spark-3.5.0-bin-hadoop3.tar.gz

# 압축 해제
tar -xzvf spark-3.5.0-bin-hadoop3.tar.gz

# /usr/local로 이동
sudo mv spark-3.5.0-bin-hadoop3 /usr/local/spark

# 소유권 변경
sudo chown -R $USER:$USER /usr/local/spark
```

**3단계: 환경변수 설정**

```bash
# .bashrc에 환경변수 추가
echo 'export SPARK_HOME=/usr/local/spark' >> ~/.bashrc
echo 'export PATH=$PATH:$SPARK_HOME/bin:$SPARK_HOME/sbin' >> ~/.bashrc
echo 'export PYSPARK_PYTHON=python3' >> ~/.bashrc

# 환경변수 적용
source ~/.bashrc
```

**4단계: Spark 설정**

```bash
# 설정 파일 복사
cd $SPARK_HOME/conf
cp spark-env.sh.template spark-env.sh
cp spark-defaults.conf.template spark-defaults.conf

# spark-env.sh 편집
nano spark-env.sh
```

**spark-env.sh 내용 추가**

```bash
#!/usr/bin/env bash

# Java 경로
export JAVA_HOME=/usr/lib/jvm/java-11-openjdk-amd64

# Spark 마스터 호스트
export SPARK_MASTER_HOST=localhost

# 메모리 설정
export SPARK_WORKER_MEMORY=4g
export SPARK_DRIVER_MEMORY=2g

# 코어 설정
export SPARK_WORKER_CORES=2
```

**spark-defaults.conf 편집**

```bash
nano spark-defaults.conf
```

**spark-defaults.conf 내용 추가**

```properties
# 기본 설정
spark.master                     spark://localhost:7077
spark.eventLog.enabled           true
spark.eventLog.dir               file:///tmp/spark-events
spark.history.fs.logDirectory    file:///tmp/spark-events

# 메모리 설정
spark.driver.memory              2g
spark.executor.memory            4g

# 파티션 설정
spark.sql.shuffle.partitions     200
```

**5단계: Spark 시작**

```bash
# 로그 디렉토리 생성
mkdir -p /tmp/spark-events

# Spark Master 시작
start-master.sh

# Spark Worker 시작
start-worker.sh spark://localhost:7077

# 실행 확인
jps

# 출력 예시:
# 12345 Master
# 12346 Worker
```

**6단계: 웹 UI 접속**

- Spark Master UI: http://localhost:8080
- Spark Worker UI: http://localhost:8081
- Spark History Server: http://localhost:18080

## 설치 방법 2: YARN 위에서 실행

Hadoop YARN이 이미 설치되어 있다면, Spark를 YARN 위에서 실행할 수 있음.

**설정 방법**

```bash
# Hadoop 설정 경로 추가
echo 'export HADOOP_CONF_DIR=$HADOOP_HOME/etc/hadoop' >> ~/.bashrc
echo 'export YARN_CONF_DIR=$HADOOP_HOME/etc/hadoop' >> ~/.bashrc
source ~/.bashrc

# spark-defaults.conf 수정
nano $SPARK_HOME/conf/spark-defaults.conf
```

**spark-defaults.conf에 YARN 설정 추가**

```properties
spark.master                     yarn
spark.submit.deployMode          client
spark.yarn.jars                  hdfs:///spark-jars/*
```

**Spark JAR 파일을 HDFS에 업로드**

```bash
# HDFS 디렉토리 생성
hdfs dfs -mkdir /spark-jars

# Spark JAR 파일 업로드
hdfs dfs -put $SPARK_HOME/jars/* /spark-jars/
```

## 설치 방법 3: Docker로 간단히 시작

**단일 컨테이너 실행**

```bash
# Spark 이미지 다운로드
docker pull apache/spark:3.5.0

# Spark Master 실행
docker run -d \
  --name spark-master \
  -p 8080:8080 \
  -p 7077:7077 \
  apache/spark:3.5.0 \
  /opt/spark/bin/spark-class org.apache.spark.deploy.master.Master

# Spark Worker 실행
docker run -d \
  --name spark-worker \
  -p 8081:8081 \
  --link spark-master:spark-master \
  apache/spark:3.5.0 \
  /opt/spark/bin/spark-class org.apache.spark.deploy.worker.Worker \
  spark://spark-master:7077

# Spark Shell 접속
docker exec -it spark-master /opt/spark/bin/spark-shell
```

**Docker Compose로 클러스터 구성**

**docker-compose.yml 파일 생성**

```yaml
version: '3'

services:
  spark-master:
    image: apache/spark:3.5.0
    container_name: spark-master
    hostname: spark-master
    ports:
      - "8080:8080"
      - "7077:7077"
      - "4040:4040"
    environment:
      - SPARK_MODE=master
      - SPARK_MASTER_HOST=spark-master
      - SPARK_MASTER_PORT=7077
      - SPARK_MASTER_WEBUI_PORT=8080
    command: /opt/spark/bin/spark-class org.apache.spark.deploy.master.Master

  spark-worker-1:
    image: apache/spark:3.5.0
    container_name: spark-worker-1
    depends_on:
      - spark-master
    ports:
      - "8081:8081"
    environment:
      - SPARK_MODE=worker
      - SPARK_MASTER_URL=spark://spark-master:7077
      - SPARK_WORKER_CORES=2
      - SPARK_WORKER_MEMORY=2g
    command: /opt/spark/bin/spark-class org.apache.spark.deploy.worker.Worker spark://spark-master:7077

  spark-worker-2:
    image: apache/spark:3.5.0
    container_name: spark-worker-2
    depends_on:
      - spark-master
    ports:
      - "8082:8081"
    environment:
      - SPARK_MODE=worker
      - SPARK_MASTER_URL=spark://spark-master:7077
      - SPARK_WORKER_CORES=2
      - SPARK_WORKER_MEMORY=2g
    command: /opt/spark/bin/spark-class org.apache.spark.deploy.worker.Worker spark://spark-master:7077

  jupyter:
    image: jupyter/pyspark-notebook:latest
    container_name: spark-jupyter
    ports:
      - "8888:8888"
    environment:
      - SPARK_MASTER=spark://spark-master:7077
    volumes:
      - ./notebooks:/home/jovyan/work
```

**클러스터 시작**

```bash
# 클러스터 시작
docker-compose up -d

# 로그 확인
docker-compose logs -f

# Jupyter 토큰 확인
docker logs spark-jupyter

# 클러스터 종료
docker-compose down
```

## 설치 방법 4: PySpark만 설치 (Python 환경)

Python에서만 Spark를 사용한다면 pip로 간단히 설치 가능함.

```bash
# PySpark 설치
pip install pyspark

# 특정 버전 설치
pip install pyspark==3.5.0

# 추가 라이브러리 설치
pip install pyspark[sql,ml,mllib]
```

**Python에서 PySpark 사용**

```python
# PySpark 설치 확인
import pyspark
print(pyspark.__version__)

# SparkSession 생성
from pyspark.sql import SparkSession

spark = SparkSession.builder \
    .appName("MyApp") \
    .master("local[*]") \
    .config("spark.driver.memory", "2g") \
    .getOrCreate()

# 간단한 데이터 처리
data = [("Alice", 25), ("Bob", 30), ("Charlie", 35)]
df = spark.createDataFrame(data, ["name", "age"])
df.show()

# Spark 종료
spark.stop()
```

## Spark Shell 사용법

**Scala Shell 시작**

```bash
# Spark Shell 실행 (Scala)
spark-shell

# Master 지정하여 실행
spark-shell --master spark://localhost:7077

# 메모리 설정
spark-shell --driver-memory 2g --executor-memory 4g
```

**Scala로 간단한 예제**

```scala
// 데이터 생성
val data = Seq(1, 2, 3, 4, 5)
val rdd = sc.parallelize(data)

// 변환 및 액션
val result = rdd.map(_ * 2).collect()
println(result.mkString(", "))

// DataFrame 생성
val df = Seq(("Alice", 25), ("Bob", 30)).toDF("name", "age")
df.show()

// SQL 쿼리
df.createOrReplaceTempView("people")
spark.sql("SELECT * FROM people WHERE age > 25").show()
```

**PySpark Shell 시작**

```bash
# PySpark Shell 실행
pyspark

# Master 지정하여 실행
pyspark --master spark://localhost:7077

# 메모리 설정
pyspark --driver-memory 2g --executor-memory 4g
```

**Python으로 간단한 예제**

```python
# RDD 생성
data = [1, 2, 3, 4, 5]
rdd = sc.parallelize(data)

# 변환 및 액션
result = rdd.map(lambda x: x * 2).collect()
print(result)

# DataFrame 생성
from pyspark.sql import Row
data = [Row(name="Alice", age=25), Row(name="Bob", age=30)]
df = spark.createDataFrame(data)
df.show()

# SQL 쿼리
df.createOrReplaceTempView("people")
spark.sql("SELECT * FROM people WHERE age > 25").show()
```

## Spark Submit으로 애플리케이션 실행

**Python 애플리케이션 실행**

```bash
# 로컬 모드
spark-submit \
  --master local[4] \
  --driver-memory 2g \
  my_app.py

# Standalone 클러스터
spark-submit \
  --master spark://localhost:7077 \
  --executor-memory 4g \
  --total-executor-cores 4 \
  my_app.py

# YARN 클러스터
spark-submit \
  --master yarn \
  --deploy-mode cluster \
  --executor-memory 4g \
  --num-executors 3 \
  my_app.py
```

**Scala/Java 애플리케이션 실행**

```bash
spark-submit \
  --class com.example.MyApp \
  --master spark://localhost:7077 \
  --executor-memory 4g \
  --total-executor-cores 4 \
  my-app.jar
```

## 설치 확인 및 테스트

**WordCount 예제 실행**

**wordcount.py 파일 생성**

```python
from pyspark.sql import SparkSession

# SparkSession 생성
spark = SparkSession.builder \
    .appName("WordCount") \
    .getOrCreate()

# 텍스트 파일 읽기
lines = spark.read.text("sample.txt").rdd.map(lambda r: r[0])

# 단어 분리 및 카운트
words = lines.flatMap(lambda line: line.split(" "))
word_counts = words.map(lambda word: (word, 1)).reduceByKey(lambda a, b: a + b)

# 결과 출력
results = word_counts.collect()
for word, count in results:
    print(f"{word}: {count}")

# Spark 종료
spark.stop()
```

**샘플 데이터 생성 및 실행**

```bash
# 샘플 텍스트 파일 생성
echo "spark hadoop spark
hadoop hive spark
spark flink hadoop" > sample.txt

# WordCount 실행
spark-submit wordcount.py
```

## Spark History Server 설정

작업 실행 이력을 보려면 History Server를 실행해야 함.

```bash
# 로그 디렉토리 생성
mkdir -p /tmp/spark-events

# History Server 시작
start-history-server.sh

# History Server 중지
stop-history-server.sh

# 웹 UI 접속
# http://localhost:18080
```

## 성능 튜닝 옵션

**spark-defaults.conf에서 성능 최적화**

```properties
# 메모리 튜닝
spark.executor.memory            4g
spark.driver.memory              2g
spark.memory.fraction            0.8
spark.memory.storageFraction     0.3

# 병렬 처리
spark.default.parallelism        200
spark.sql.shuffle.partitions     200

# 직렬화
spark.serializer                 org.apache.spark.serializer.KryoSerializer
spark.kryoserializer.buffer.max  256m

# 압축
spark.rdd.compress               true
spark.shuffle.compress           true
```

## 문제 해결

**메모리 부족 오류**

```bash
# Executor 메모리 증가
spark-submit --executor-memory 8g my_app.py

# Driver 메모리 증가
spark-submit --driver-memory 4g my_app.py
```

**포트 충돌**

```bash
# 다른 포트 사용
start-master.sh --port 7078 --webui-port 8090
```

**Python 버전 충돌**

```bash
# 특정 Python 버전 지정
export PYSPARK_PYTHON=python3.9
export PYSPARK_DRIVER_PYTHON=python3.9
```

## Spark 중지

```bash
# Worker 중지
stop-worker.sh

# Master 중지
stop-master.sh

# 전체 중지
stop-all.sh

# History Server 중지
stop-history-server.sh
```
