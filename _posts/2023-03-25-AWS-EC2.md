...
* AWS에서 제공하는 서비스형 Infrastructure (IaaS) 입니다. (laaS = Infrastructure as a Service)
...
### Compute Optimized
* 고성능 처리가 필요한 애플리케이션, CPU 위주의 배치 처리 작업, 높은 처리량 기반 애플리케이션, 기계 학습 및 고성능 과학 계산 작업에 적합합니다.
...
* 보안 그룹은 AWS 클라우드에서 네트워크 보안 실행의 핵심이며 EC2 인스턴스에 들어오고 나가는 트래픽을 제어합니다.
* 보안 그룹은 허용 규칙(Allow rule)만 포함합니다.
...
* 보안 그룹과 인스턴스 간은 n:n 관계입니다.
* 리전과 VPC 결합으로 통제할 수 있습니다.
...
| 구매 옵션              | 설명                                                         |
| ---------------------- | ------------------------------------------------------------ |
| On demand              | - 단기적인 워크로드에 적합 <br/>- 비용 예측 가능, 초 단위로 요금 지불 가능 |
...
| Savings Plans          | - 달러 단위로 특정한 사용량 약정<br/>- 장기 워크로드 (70% 까지 할인 받을 수 있음) 시간당 10달러로 약정 <br/>- 사용량이 한도 넘어가면 온디맨드 가격으로 청구되며, 특정한 인스턴스, 패밀리, 리전으로 고정 |
...
| Dedicated Hosts        | - 실제 물리적 서버를 받게 됨 (물리적 서버 자체에 대한 접근권을 갖고, 낮은 수준의 하드웨어 대한 가시성을 제공해 줌)<br/>- 법규 준수 요건이 있는 활용 사례나 소켓, 코어, VM 소프트웨어 라이선스를 기준으로 청구되는 기존 서버에 연결된 소프트웨어 라이선스가 있는 경우 -> 물리적 서버에 접근하고 전용 호스트를 갖춰서 활용하기 적합<br/>- 온디맨드로 초당 비용 지불하거나 1년 또는 3년동안 예약 가능<br/>- 가장 비싼 옵션 |
